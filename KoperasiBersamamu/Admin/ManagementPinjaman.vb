Public Class ManagementPinjaman
    ...
    Private Sub data_grid_anggota_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles data_grid_anggota.CellClick
        If e.RowIndex >= 0 Then
            ...
            Dim sql As String = "SELECT tbl_peminjaman.id_peminjaman as [ID Peminjaman], jumlah_peminjaman, kali_angsur, jumlah_angsur, bunga, total_bunga, total_angsur FROM tbl_total_bunga INNER JOIN tbl_peminjaman ON tbl_peminjaman.id_peminjaman = tbl_total_bunga.id_peminjaman WHERE id_anggota = '" + id_anggota + "' AND is_lunas = 0"
            Dim adapter As New SqlClient.SqlDataAdapter(sql, conn)
            Dim dt As New DataTable
    ...
    Private Sub data_grid_pinjaman_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles data_grid_pinjaman.CellClick
        If e.RowIndex >= 0 Then
            Dim id As String = data_grid_pinjaman.Rows(e.RowIndex).Cells(0).Value
            DetailAngsuran.Tag = id
            DetailAngsuran.Show()
        End If
    End Sub
End Class