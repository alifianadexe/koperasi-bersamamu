...
Public Class SimpanForm
    ...
    Private Sub print_page(ByVal sender As Object, ByVal ev As PrintPageEventArgs)
        ...
        gp.DrawString("              Bukti Simpanan Anggota        ", New Font("Consolas", 16), New SolidBrush(Color.Black), x + img.Width, y + offset)
        offset += fontHeight
        gp.DrawString("----------------------------------------------------------------", New Font("Consolas", 16), New SolidBrush(Color.Black), x, y + offset)
        offset += fontHeight + 10
        'end header

        Dim sql As String = "SELECT nama,tbl_simpanan.id_simpanan as [id_simpanan], tbl_det_simpanan.id_det_simpanan as [id_det_pinjaman], saldo, jenis_simpanan, tanggal_transaksi, jumlah_simpan, total FROM ((tbl_simpanan INNER JOIN tbl_anggota ON tbl_simpanan.id_anggota = tbl_anggota.id_anggota) INNER JOIN tbl_det_simpanan ON tbl_det_simpanan.id_simpanan = tbl_simpanan.id_simpanan )INNER JOIN tbl_jenis_simpanan ON tbl_jenis_simpanan.id_jenis_simpanan = tbl_det_simpanan.id_jenis_simpanan WHERE tbl_simpanan.id_simpanan = '" + Me.lbl_simpanan.Text + "' ORDER BY id_det_simpanan DESC"
        Dim cmnd As New SqlClient.SqlCommand(sql, conn)
        ...
        If rd.HasRows Then
            ...
            Dim jumlah As String = ("Jumlah Simpan     : " + Format(rd.Item("jumlah_simpan"), "Rp ##,##0.00")).ToString.PadRight(50)
